  ...
  return (
    <div className="relative min-h-screen z-10 flex flex-col justify-between p-8 md:p-16">
      ...
      <header className="max-w-2xl">
        <span className="text-xs uppercase tracking-widest text-(--g-brand) font-semibold mb-2 block">
          Phase 3 Dev & Verification
        </span>
        <h1 className="text-3xl md:text-5xl font-bold tracking-tight mb-4 text-(--g-ink) font-(family-name:--font-klee-one)">
      ...
      <main className="my-12 max-w-xl">
        <div className="bg-(--white-veil) p-6 rounded-3xl shadow-(--shadow-soft) border border-[color-mix(in_oklab,var(--g-ink)_10%,transparent)] backdrop-blur-md">
            ...
            {GROUPS.map((g) => (
              <button
                ...
                className={`px-4 py-2.5 rounded-full text-sm font-medium transition-all duration-300 ${
                  selectedGroup === g.id
                    ? 'bg-(--g-brand) text-white shadow-md scale-105'
                    : 'bg-white/80 hover:bg-white text-(--ink) border border-[color-mix(in_oklab,var(--g-ink)_15%,transparent)]'
                }`}
              >
                {g.name}
              </button>
          ...
          <div className="mt-6 pt-4 border-t border-[color-mix(in_oklab,var(--g-ink)_10%,transparent)] flex items-center justify-between text-xs text-(--ink-soft)">
      ...
      <footer className="flex justify-between items-center text-xs text-(--ink-soft)">
