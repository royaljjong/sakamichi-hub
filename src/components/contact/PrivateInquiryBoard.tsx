...
const inputClass = 'mt-2 w-full rounded-xl border border-(--border-soft) bg-white px-4 py-3 font-normal';
...
export function PrivateInquiryBoard({ locale }: { locale: string }) {
  ...
  if (!isPrivateInquiryEnabled()) return <div className="rounded-2xl border border-(--border-soft) bg-(--surface-card) p-6 text-sm text-(--ink-soft)">{t.setup}</div>;
  ...
  const accessFields = <>
    <label className="block text-sm font-semibold">{t.privateId}<input maxLength={40} autoComplete="off" value={privateId} onChange={(e) => setPrivateId(e.target.value)} className={inputClass} /><span className="mt-1 block text-xs font-normal text-(--ink-soft)">{t.idHelp}</span></label>
    <label className="block text-sm font-semibold">{t.password}<input type="password" minLength={8} maxLength={128} autoComplete="new-password" value={password} onChange={(e) => setPassword(e.target.value)} className={inputClass} /><span className="mt-1 block text-xs font-normal text-(--ink-soft)">{t.passwordHelp}</span></label>
  </>;

  return <section className="space-y-6">
    <p className="rounded-xl bg-[color-mix(in_oklab,var(--g-brand)_9%,white)] p-4 text-sm text-(--ink-soft)">{t.privacy}</p>
    <div className="flex gap-2" role="tablist">
      <button id="inquiry-create-tab" type="button" role="tab" aria-selected={mode === 'create'} aria-controls="inquiry-create-panel" tabIndex={mode === 'create' ? 0 : -1} onKeyDown={handleTabKeyDown} onClick={() => selectMode('create')} className={`rounded-full px-4 py-2 text-sm font-semibold ${mode === 'create' ? 'bg-(--g-brand) text-white' : 'bg-(--paper-deep)'}`}>{t.createTab}</button>
      <button id="inquiry-lookup-tab" type="button" role="tab" aria-selected={mode === 'lookup'} aria-controls="inquiry-lookup-panel" tabIndex={mode === 'lookup' ? 0 : -1} onKeyDown={handleTabKeyDown} onClick={() => selectMode('lookup')} className={`rounded-full px-4 py-2 text-sm font-semibold ${mode === 'lookup' ? 'bg-(--g-brand) text-white' : 'bg-(--paper-deep)'}`}>{t.lookupTab}</button>
    </div>
    {mode === 'create' ? <div id="inquiry-create-panel" role="tabpanel" aria-labelledby="inquiry-create-tab"><form onSubmit={handleCreate} className="space-y-4">{accessFields}<label className="block text-sm font-semibold">{t.category}<select value={category} onChange={(e) => setCategory(e.target.value)} className={inputClass}><option value="takedown">{t.takedown}</option><option value="correction">{t.correction}</option><option value="general">{t.general}</option></select></label><label className="block text-sm font-semibold">{t.title}<input maxLength={120} value={title} onChange={(e) => setTitle(e.target.value)} className={inputClass} /></label><label className="block text-sm font-semibold">{t.body}<textarea maxLength={5000} rows={7} value={body} onChange={(e) => setBody(e.target.value)} className={`${inputClass} resize-y`} /></label><button disabled={busy} className="rounded-xl bg-(--g-brand) px-5 py-3 text-sm font-bold text-white disabled:opacity-60">{busy ? t.working : t.submit}</button></form></div> : <div id="inquiry-lookup-panel" role="tabpanel" aria-labelledby="inquiry-lookup-tab" className="space-y-6"><form onSubmit={handleLookup} className="space-y-4">{accessFields}<button disabled={busy} className="rounded-xl bg-(--g-brand) px-5 py-3 text-sm font-bold text-white disabled:opacity-60">{busy ? t.working : t.lookup}</button></form><div className="space-y-4">{inquiries.map((item) => <article key={item.id} className="rounded-2xl border border-(--border-soft) bg-(--surface-card) p-5"><div className="flex items-start justify-between gap-3"><h3 className="font-bold">{item.title}</h3><span className="count-pill">{item.status}</span></div><time className="mt-1 block text-xs text-(--ink-soft)">{new Intl.DateTimeFormat(lang, { dateStyle: 'medium' }).format(new Date(item.created_at))}</time><p className="mt-4 whitespace-pre-wrap text-sm leading-relaxed">{item.body}</p><div className="mt-5 rounded-xl bg-(--paper-deep) p-4"><strong className="text-sm">{t.reply}</strong><p className="mt-2 whitespace-pre-wrap text-sm text-(--ink-soft)">{item.admin_reply || t.noReply}</p></div></article>)}</div></div>}
    {message && <p role="alert" className="text-sm text-(--g-brand)">{message}</p>}
  </section>;
}
