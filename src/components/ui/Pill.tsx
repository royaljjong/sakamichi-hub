...
export function Pill({
  ...
  let variantStyle = 'bg-(--white-veil) text-(--ink) border border-[color-mix(in_oklab,var(--g-ink)_15%,transparent)] shadow-xs';
  if (variant === 'outline-solid') {
    variantStyle = 'border border-(--ink-faint) text-(--ink-soft) hover:text-(--ink)';
  } else if (variant === 'subtle') {
    variantStyle = 'bg-[color-mix(in_oklab,var(--g-brand)_12%,var(--paper))] text-(--g-ink)';
  }
