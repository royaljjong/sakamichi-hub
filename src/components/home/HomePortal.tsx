...
const COPY = {
  ja: { title: '坂道シリーズと48グループ', lead: 'グループから、メンバーと公式SNSへ。', saka: '坂道シリーズ', akb: 'AKB48グループ', groups: 'グループを選ぶ', events: 'ライブ・公演', birthdays: '今月の誕生日', emptyEvent: '確認済みの今後のライブはありません', emptyBirthday: '今月の誕生日メンバーはいません', member: 'メンバー', active: '現役', updated: '公式情報を毎日更新' },
  ko: { title: '사카미치와 AKB48 그룹', lead: '그룹에서 멤버, 그리고 공식 SNS까지.', saka: '사카미치', akb: 'AKB48 그룹', groups: '그룹 선택', events: '라이브·공연', birthdays: '이달의 생일자', emptyEvent: '검증된 예정 라이브가 없습니다', emptyBirthday: '이번 달 생일자가 없습니다', member: '멤버', active: '현재', updated: '공식 정보를 매일 업데이트' },
  en: { title: 'Sakamichi & AKB48 Group', lead: 'From group to member to official social links.', saka: 'Sakamichi Series', akb: 'AKB48 Group', groups: 'Choose a group', events: 'Live & shows', birthdays: 'Birthdays this month', emptyEvent: 'No verified upcoming live shows', emptyBirthday: 'No birthdays this month', member: 'Members', active: 'Active', updated: 'Updated daily from official sources' },
};
  ...
  return <div className="space-y-12 pb-8">
    ...
    <section>
      <div className="grid gap-4 sm:grid-cols-2">
        {(['sakamichi', 'akb48g'] as Family[]).map((item) => <button key={item} type="button" onClick={() => selectFamily(item)} onMouseEnter={() => document.documentElement.setAttribute('data-group', item === 'sakamichi' ? 'nogizaka46' : 'akb48')} className={`family-gate ${family === item ? 'family-gate-active' : ''}`}><span className="family-wordmark" aria-hidden="true"><b>{item === 'sakamichi' ? '坂道' : 'AKB48'}</b><em>{item === 'sakamichi' ? 'SERIES' : 'GROUP'}</em></span><strong>{item === 'sakamichi' ? copy.saka : copy.akb}</strong><small>{groups.filter((g) => g.franchise === item).length} GROUPS</small></button>)}
      </div>
    </section>

    <section className="editorial-panel p-5 sm:p-7"><p className="section-kicker">01 · {copy.groups}</p><h2 className="section-title mt-2">{family === 'sakamichi' ? copy.saka : copy.akb}</h2><div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{familyGroups.map((group) => { const active = members.filter((m) => m.primaryGroupId === group.id && (m.status === 'active' || m.status === 'graduating')).length; return <Link key={group.id} href={`/g/${group.id}`} onMouseEnter={() => document.documentElement.setAttribute('data-group', group.id)} className="group-tile"><span className="text-[10px] uppercase tracking-[.14em] text-[var(--ink-soft)]">{group.franchise === 'sakamichi' ? copy.saka : copy.akb}</span><strong className="mt-5 block text-xl tracking-tight">{localized(group.name)}</strong><span className="mt-2 block text-xs text-[var(--ink-soft)]">{copy.active} {active} · {copy.member} {members.filter((m) => m.primaryGroupId === group.id).length}</span></Link>; })}</div></section>

    {(['sakamichi', 'akb48g'] as Family[]).map((target) => { const events = eventsFor(target); return <section key={`events-${target}`} className="editorial-panel p-5 sm:p-7"><div className="flex items-end justify-between gap-4"><div><p className="section-kicker">{target === 'sakamichi' ? copy.saka : copy.akb}</p><h2 className="section-title mt-2">{copy.events}</h2><p className="mt-2 text-[11px] text-[var(--ink-soft)]">{copy.updated} · {portal.generatedAt}</p></div><span className="count-pill">{events.length}</span></div>{events.length ? <Rail label={`${target} events`}>{events.map((event) => { const group = groupMap.get(event.groupIds[0] ?? ''); const venue = event.venueId ? venues.get(event.venueId) : undefined; return <a key={event.id} href={event.ticketUrl ?? event.officialUrl} target="_blank" rel="noopener noreferrer" className="event-poster-card group" onMouseEnter={() => group && document.documentElement.setAttribute('data-group', group.id)}><div className="event-poster-media">{event.posterUrl ? <img src={event.posterUrl} alt="" className="h-full w-full object-cover" /> : <div className="event-poster-placeholder" style={{ '--event-color': group?.palette.brand } as React.CSSProperties}><span>{group ? localized(group.shortName) : ''}</span><b>LIVE</b></div>}</div><div className="p-4"><time className="text-[11px] text-[var(--ink-soft)]">{new Intl.DateTimeFormat(lang, { month: 'short', day: 'numeric', weekday: 'short' }).format(new Date(event.startsAt))}</time><h3 className="mt-2 line-clamp-2 text-sm font-semibold">{localized(event.title)}</h3><p className="mt-2 truncate text-[11px] text-[var(--ink-soft)]">{venue ? localized(venue.name) : localized(group?.baseLocation ?? { ja: 'オンライン', ko: '온라인', en: 'Online' })} ↗</p></div></a>; })}</Rail> : <p className="mt-6 border-t border-black/10 py-8 text-sm text-[var(--ink-soft)]">{copy.emptyEvent}</p>}</section>; })}
