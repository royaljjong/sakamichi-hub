...
export function HomePortal({ groups, members, locale, portal, videos }: { groups: Group[]; members: Member[]; locale: string; portal: PortalDataset; videos: MemberVideo[] }) {
  ...
  return <div className="space-y-12 pb-8">
    ...
    <section id="section-family" className="border-b border-black/10 pb-10 pt-6"><p className="section-kicker">OFFICIAL IDOL DIRECTORY</p><h1 className="mt-5 max-w-5xl text-4xl font-semibold tracking-[-.045em] text-[var(--g-ink)] sm:text-6xl lg:text-7xl">{t('title')}</h1><p className="mt-5 text-sm text-[var(--ink-soft)] sm:text-base">{t('lead')}</p></section>
    ...
    <section id="section-live">{(['sakamichi', 'akb48g'] as Family[]).map((target) => { const events = eventsFor(target); return <div key={`events-${target}`} className="editorial-panel p-5 sm:p-7 mb-6 last:mb-0"><div className="flex items-end justify-between gap-4"><div><p className="section-kicker">{target === 'sakamichi' ? tFranchise('sakamichiShort') : tFranchise('akb48gShort')}</p><h2 className="section-title mt-2">{t('events')}</h2><p className="mt-2 text-[11px] text-[var(--ink-soft)]">{t('updated')} · {portal.generatedAt}</p></div><span className="count-pill">{events.length}</span></div>{events.length ? <Rail label={`${target} events`}>{events.map((event, i) => { const group = groupMap.get(event.groupIds[0] ?? ''); const venue = event.venueId ? venues.get(event.venueId) : undefined; return <div key={event.id} className="stagger-item" style={{ '--i': Math.min(i, 12) } as React.CSSProperties}><a href={event.ticketUrl ?? event.officialUrl} target="_blank" rel="noopener noreferrer" className="event-poster-card group" onMouseEnter={() => group && setPreview(group.id)} onMouseLeave={clearPreview}><div className="event-poster-media">{event.posterUrl ? <img src={event.posterUrl} alt="" className="h-full w-full object-cover" /> : <div className="event-poster-placeholder" style={{ '--event-color': group?.palette.brand } as React.CSSProperties}><span>{group ? localized(group.shortName) : ''}</span><b>LIVE</b></div>}</div><div className="p-4"><time className="text-[11px] text-[var(--ink-soft)]">{new Intl.DateTimeFormat(lang, { month: 'short', day: 'numeric', weekday: 'short' }).format(new Date(event.startsAt))}</time><h3 className="mt-2 line-clamp-2 text-sm font-semibold">{localized(event.title)}</h3><p className="mt-2 truncate text-[11px] text-[var(--ink-soft)]">{venue ? localized(venue.name) : localized(group?.baseLocation ?? { ja: 'オンライン', ko: '온라인', en: 'Online' })} ↗</p></div></a></div>; })}</Rail> : <p className="mt-6 border-t border-black/10 py-8 text-sm text-[var(--ink-soft)]">{t('emptyEvent')}</p>}</div>; })}</section>
